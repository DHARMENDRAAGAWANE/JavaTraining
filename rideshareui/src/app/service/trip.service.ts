  ...
  httpOptions = {
    ...
    "Access-Control-Allow-Origin" : "*"}
      ...
      constructor(private _http:HttpClient,private _configuration:Configuration ,private messageService: MessageService) {
          ...
          this.actionUrl = _configuration.ServerWithApiUrl +'/trip/';
      ...
      saveTrip(trip: Trip): Observable<Trip> {
        ...
        return this._http.post<Trip>(this.actionUrl, trip, this.httpOptions).pipe(
          tap(),
          catchError(this.handleError<Trip>('create Trip'))
        );
      }
  
      searchTrips(from:String,to:String,time:String): Observable<Trip[]> {
       var searchURl= this.actionUrl+'search?'+'source='+from+'&destination='+to+'&time='+time;
        return this._http.get<Trip[]>(searchURl,  this.httpOptions).pipe(
          tap(),
          catchError(this.handleError<Trip[]>('create Trip'))
        );
      }
